...
async function logToFlodesk(email, firstName) {
  ...
  try {
    ...
    if (SEGMENT && (up.status === 200 || up.status === 201)) {
      const segPath = "/v1/subscribers/" + encodeURIComponent(email.trim()) + "/segments";
      console.log("Flodesk segment path:", segPath, "payload:", JSON.stringify({ segment_ids: [SEGMENT] }));
