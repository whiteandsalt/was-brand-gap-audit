...
// Log lead to Flodesk (direct integration, non-blocking)
async function logToFlodesk(email, firstName) {
  const KEY = process.env.FLODESK_API_KEY || "";
  const SEGMENT = process.env.FLODESK_SEGMENT_ID || "";
  if (!KEY || !validEmail(email)) {
    console.log("Flodesk skipped. key present:", !!KEY, "valid email:", validEmail(email));
    return;
  }
  const auth = "Basic " + Buffer.from(KEY + ":").toString("base64");
  try {
    const up = await post("api.flodesk.com", "/v1/subscribers",
      { Authorization: auth },
      { email: email.trim(), first_name: (firstName || "").trim() || undefined }
    );
    console.log("Flodesk upsert status:", up.status);
    if (SEGMENT) {
      const seg = await post("api.flodesk.com",
        "/v1/subscribers/" + encodeURIComponent(email.trim()) + "/segments",
        { Authorization: auth },
        { segment_ids: [SEGMENT] }
      );
      console.log("Flodesk segment status:", seg.status);
    }
  } catch (e) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === "send-email") {
      ...
      // Log lead to Flodesk (non-blocking, after email)
